...
def evaluate_sclp_phase(group, test, phase_name,phase_name_flag, output_file_type):
    ...
    for idx, tc in enumerate(glob.glob("{}*.c".format(test))):
        try:
            sr = subprocess.run(["./Submissions/{}/sclp".format(group), tc, phase_name_flag], timeout=TESTCASE_TIMEOUT_PARAM, stdout = output_log_file, stderr = output_log_file)
...
if __name__ == '__main__':
    ...
    for submission in submissions:
        ...
        if validate_submission_name(tar_file_name):
            ...
            if build_project(group):
                ...
                if args.csv:
                    ...
                    eval_result_fail.extend([0 for i in range(1,len(df[0]))])
        ...
        print("\n\n\n")
    if args.csv:
        with open("{}_Evaluation_Result.csv".format(args.assignment), 'w') as f:
            write = csv.writer(f)
            for row in df:
                write.writerow(row)
