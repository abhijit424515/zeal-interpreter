...
%token INTEGER FLOAT VOID STRING BOOL READ WRITE ASSIGN NOT EQ NE GT LT AND GE LE OR
...
program
    : global_decl_statement_list_with_func func_def             { if (!stop_after_parse) print_program_ast(); if (!stop_after_ast) print_program_tac(); }
    | global_var_decl_stmt_list func_def                        { if (!stop_after_parse) print_program_ast(); if (!stop_after_ast) print_program_tac(); }
    | func_def                                                  { if (!stop_after_parse) print_program_ast(); if (!stop_after_ast) print_program_tac(); }
;
