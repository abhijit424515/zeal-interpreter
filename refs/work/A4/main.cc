...
int main (int argc, char **argv) {
	struct argp_option options[] = {
		...
		{ "suppress-comments",  's' , 0, 0, "Suppress comments in RTL and Assembly files", 2 },
...
static int parse_opt (int key, char *arg, struct argp_state *state) {
	switch (key) {
		...
		case 'P': 
			...
			break;
		case 'A': 
			stop_after_ast = true; stop_after_tac = true; stop_after_rtl = true;
			break;
		case 'C': 
			stop_after_tac = true; stop_after_rtl = true;	
			break;
		case 'R': 
			stop_after_rtl = true;	
			break;
