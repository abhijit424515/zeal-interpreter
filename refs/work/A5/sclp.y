...
%token INTEGER FLOAT VOID STRING BOOL READ WRITE ASSIGN NOT EQ NE GT LT AND GE LE OR IF ELSE WHILE DO RETURN
...
%type <exp> expression rel_expression if_condition actual_arg;
%type <cst> constant_as_operand;
%type <name> var_decl_item variable_name variable_as_operand;
%type <name_list> var_decl_item_list;
%type <stm> statement assignment_statement if_statement do_while_statement while_statement compound_statement read_statement print_statement return_statement call_statement;
...
%type <fun> func_def; 
%type <exp_list> actual_arg_list non_empty_arg_list;
%type <fc> func_call;
%start program;
...
program
    : global_decl_statement_list func_def_list      { if (!stop_after_parse) init(); }
    | func_def_list                                 { if (!stop_after_parse) init(); }
;
...
statement
    ...
    | call_statement                                { if (!stop_after_parse) $$ = $1; }
    | return_statement                              { if (!stop_after_parse) $$ = $1; }
;

call_statement
    : func_call ';'                                 { if (!stop_after_parse) $$ = process_fcall($1); }
;

func_call
    : NAME '(' actual_arg_list ')'                  { if (!stop_after_parse) $$ = create_fcall($1, $3); }
;

actual_arg_list
    : non_empty_arg_list                            { if (!stop_after_parse) $$ = $1; }
    | %empty                                        { if (!stop_after_parse) $$ = add_to_arg_list(NULL, NULL); }
;

non_empty_arg_list
    : non_empty_arg_list ',' actual_arg             { if (!stop_after_parse) $$ = add_to_arg_list($1, $3); }
    | actual_arg                                    { if (!stop_after_parse) $$ = add_to_arg_list(NULL, $1); }
;
...
return_statement
    : RETURN expression ';'                         { if (!stop_after_parse) $$ = process_return($2); }
;
...
assignment_statement
    ...
    | variable_as_operand ASSIGN func_call ';'      { if (!stop_after_parse) $$ = process_assign_fcall($1, $3); }
;
