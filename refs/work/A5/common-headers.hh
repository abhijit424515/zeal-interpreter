...
struct fcall {
	...
	fcall(string* name = NULL, list<Expression*> *l = NULL): name(name), l(l) {}
};
...
struct TAC_Statement {
	...
	TAC_Statement(string *lhs, op_type op = PLUS_O, string *o1 = NULL, string *o2 = NULL, tac_type t = NARY_TAC, tmp_type lhs_t = SOURCE_T, tmp_type o1_t = SOURCE_T, tmp_type o2_t = SOURCE_T, var_type lhs_v = INTEGER_V, var_type rhs_v = INTEGER_V, fcall *fc = NULL): lhs(lhs), op(op), o1(o1), o2(o2), t(t), lhs_t(lhs_t), o1_t(o1_t), o2_t(o2_t), lhs_v(lhs_v), rhs_v(rhs_v), fc(fc) {}
	void print_tac();
};
...
struct Statement {
	...
	Statement(string *s = NULL, Expression *e = NULL, statement_type t = ASSIGN_S, var_type v = INTEGER_V, Statement *s1 = NULL, Statement *s2 = NULL, list<Statement*> *sl = NULL, fcall *fc = NULL): s(s), e(e), t(t), v(v), s1(s1), s2(s2), sl(sl), fc(fc) {}
...
struct asm_entry {
	...
	asm_entry(reg r, int offset, bool addr): r(r), offset(offset), addr(addr) {}
	string to_string();
};
...
struct aarg {
	...
	aarg(reg r, int offset, bool addr) {
		z.y = new asm_entry(r,offset,addr);
		used = 1;
	}

	string to_string() {
		string *p = z.x;
		if (used == 0 && p != NULL && *p == "zero") {
			p = new string("$zero");
		}  
		return (used == 0) ? *p : z.y->to_string();
	}
};
...
struct ASM {
	...
	ASM(asm_instr i, bool is_int = true, aarg a1 = aarg(NULL), aarg a2 = aarg(NULL), aarg a3 = aarg(NULL)): i(i),is_int(is_int),a1(a1),a2(a2),a3(a3) {}
	void print_asm();
};
