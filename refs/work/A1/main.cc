...
static int parse_opt (int key, char *arg, struct argp_state *state) {
	switch (key) {
		case 't': 
			...
			break;
		case 'a':
			cerr << "sclp error: File:  The AST related options are not supported. The current implementation supports only SCAN and PARSE.\n";
			exit(1);
			break;
		case 'c':
			cerr << "sclp error: File:  The TAC related options are not supported. The current implementation supports only SCAN and PARSE.\n";
			exit(1);
			break;
		case 'r':
...
void store_token_name(string token_name, char *lexeme, int lineno) {
	if (show_tokens) {
		...
		if (!demo_mode) {
			if ((fd = open(new_filename, O_WRONLY | O_CREAT | O_TRUNC, 0644)) == -1) {
